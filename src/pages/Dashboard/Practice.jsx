...
const Practice = () => {
    ...
    const fetchData = async () => {
      try {
        const [testsRes, attemptsRes] = await Promise.all([
          api.get('/tests'), // In a real app, this would be /api/practice
  ...
  if (loading) return <div className="text-center text-white">Loading Practice Arena...</div>;

  return (
    <div>
      <h1 className="text-4xl font-bold text-white mb-8">Practice Arena</h1>
      
      <h2 className="text-2xl font-bold text-white mb-4">Available Challenges</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {available.map(item => (
          <ContentCard key={item._id} {...item} onClick={() => alert(`Starting: ${item.title}`)} />
        ))}
        {available.length === 0 && <p className="text-gray-400">No new challenges available. Great job!</p>}
      </div>

      <h2 className="text-2xl font-bold text-white mt-12 mb-4">Completed Challenges</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {completed.map(item => (
          <ContentCard key={item._id} {...item} status="Completed" onClick={() => alert(`Reviewing: ${item.title}`)}/>
        ))}
         {completed.length === 0 && <p className="text-gray-400">You haven't completed any challenges yet.</p>}
      </div>
    </div>
  );
};
